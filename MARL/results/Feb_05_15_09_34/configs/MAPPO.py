...
from llm_controller.merge_env import *
from llm_controller.llm_agent import *
...
class MAPPO:
    ...
    def __init__(self, env, state_dim, action_dim,
        ...
        self.env_state, self.action_mask = self.env.reset() #获取veichle 位置，状态
        self.n_episodes = 0
        self.n_steps = 0
    ...
    def interact(self):
        ...
        # take n steps
        for i in range(self.roll_out_n_steps):  #sample 100
            # availableActions = self.env.get_available_actions()
            states.append(self.env_state)
            llm = True
            rl = True
            if llm:#for
                llm_agent = LlmAgent(self.env.road)  # create llmagent
                sce = llm_agent.retrun_sce()  # sce data
                llm_actions = llm_agent.llm_controller_run(self.env, self.env.controlled_vehicles)
            ...
            if rl:#for rl
                ...
                next_state, global_reward, done, info = self.env.step(tuple(action), self.env)
                print("rl_actions:", action)
                print("global_reward_rl:", global_reward)
            ...
            if done:
                ...
                break
        # 计算每个子数组的平均值并存储在列表中
        # 使用列表推导式将所有值加起来，并统计总的元素数量
        total_sum = sum(item for sublist in rewards for item in sublist)
        total_count = sum(len(sublist) for sublist in rewards)
        print("eposido_reward:", total_sum/total_count)
