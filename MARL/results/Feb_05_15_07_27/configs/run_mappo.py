...
def parse_args():
    ...
    parser.add_argument('--model-dir', type=str, required=False,
                        default='results/Jan_31_14_51_55', help="pretrained model path")
...
def train(args):
    ...
    while mappo.n_episodes < MAX_EPISODES:
        print("episodes:", mappo.n_episodes)
        mappo.interact()
        print("interact done")
...
def evaluate(args):
    ...
    state_dim = env.n_s
    action_dim = env.n_a
    eval_episodes = 1 #inference times
    # This line of code stores the command line argument for evaluation seeds in the test_seeds variable
    test_seeds = args.evaluation_seeds
    seeds = [int(s) for s in test_seeds.split(',')]
    seeds = seeds[:eval_episodes]
    mappo = MAPPO(env=env, memory_capacity=MEMORY_CAPACITY,
                  state_dim=state_dim, action_dim=action_dim,
                  batch_size=BATCH_SIZE, entropy_reg=ENTROPY_REG,
                  roll_out_n_steps=ROLL_OUT_N_STEPS,
