...
class MergeEnv(AbstractEnv):
    ...
    # def _reward(self, action: list) -> float: #src
    # # Cooperative multi-agent reward
    ...
    def _agent_reward(self, action: int, vehicle: Vehicle, obs, road, env) -> float:
       ...
       """
        ...
        #llm reward
        llm = False
        if llm:#action created by llm
        #input- env, self.controlled_vehicles;
        #output- llm_actions
        # if vehicle.lane_index == ("b", "c", 1):#only run llm in merge lane
            llm_agent = LlmAgent(env.road)  # create llmagent
            sce = llm_agent.retrun_sce()  # sce data
            llm_actions = llm_agent.llm_controller_run(env, self.controlled_vehicles)
        if llm: #calculate reward
            rewards = []
            llm_reward = self.calculate_custom_reward(action, llm_actions)
